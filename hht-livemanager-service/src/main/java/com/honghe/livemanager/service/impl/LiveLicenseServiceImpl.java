...
public class LiveLicenseServiceImpl implements LiveLicenseService {
    ...
    @Autowired
    private LiveDistrictService liveDistrictService;
...
//            System.out.println("file is not exists.fileAutoName:"+fileAutoName);
    ...
    private String getAddress(LiveLicense liveLicense){
        ...
        String districtName="";
        districtName=liveDistrictService.selectById(liveLicense.getProvinceId(),0,1).getName();
        address.append(districtName).append(",");
        districtName=liveDistrictService.selectById(liveLicense.getCityId(),liveLicense.getProvinceId(),2).getName();
        address.append(districtName).append(",");

        districtName=liveDistrictService.selectById(liveLicense.getCountyId(),liveLicense.getCityId(),3).getName();

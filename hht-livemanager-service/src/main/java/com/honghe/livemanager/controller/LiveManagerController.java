...
public class LiveManagerController {
    ...
    public Result exportLiveList(String currentDate,String status,String beginTime,String endTime){
        ...
        if(liveList!=null && !liveList.isEmpty()) {
            String[] headers = {"学校名称","直播名称","主讲人","计划直播时间","实际直播时间","推流设备IP","推流地址",
                    "RTMP播放流地址","FLV播放流地址","HLS播放流地址","推流帧率Fps","推流码率KB/S","带宽Mbps","流量GB","观看人数峰值","直播状态"};
            Date date=new Date();
     ...
     * @param status       0 表示禁用，1 表示允许推流
     * @param time         过期时间，格式:yyyy-MM-dd HH:mm:ss
     * @return
     */
    @MyLog("直播管理-开启/关闭推流")
    @RequestMapping(value="setLiveChannelStatus",method=RequestMethod.GET)
    public Result setLiveChannelStatus(String streamCode,String status,String time){
        if(ParamUtil.isEmpty(streamCode) || ParamUtil.isEmpty(status)){
            return ConvertResult.getParamErrorResult();
        }
        JSONObject re_value = cloudTencentApi.setLiveStatus(streamCode,status,time,null,null);
        Result result=new Result();
        if(re_value!=null && !re_value.isEmpty() &&
            ...
            //直播关闭
            if("0".equals(status)){
                //保存直播实际结束时间及更新status=3
                String liveStatus=String.valueOf(Constants.LIVE_OVER_STATUS);

                liveService.updateLiveByStreamCode(liveStatus,streamCode,null,DateUtil.currentDatetime(),0);
//                //保存直播宽带、在线人数、推流ip等统计数据
//                liveService.updateLiveHistoryByStreamCode(streamCode);
            }
        }else{
    ...
    public Result getLiveStatus(String streamCode,String time){
        ...
        JSONObject re_value=cloudTencentApi.getStatus(streamCode,time);
        Result result=new Result();
     ...
     * @date 20190304
     */
    @RequestMapping(value = "searchLiveStatistic",method = RequestMethod.GET)
    public Result searchLiveStatistic(int liveId){
        return liveService.searchLiveStatistic(liveId);
    }
}
